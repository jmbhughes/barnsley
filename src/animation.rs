...
impl AnimationSequence {
    pub fn animate(&self, width: usize, height: usize, num_iterations: usize, num_points: usize) -> Vec<Image> {
        ...
        for pair_index in 1..self.ifs_vec.len() {
            let num_steps_for_pair = *self.step_counts.get(pair_index-1).unwrap();
            for step in 0..num_steps_for_pair{
                let start = self.ifs_vec.get(pair_index - 1).unwrap();
                let end = self.ifs_vec.get(pair_index).unwrap();
                ...
                images.insert(pair_index-1, this_image);
            }
        }
